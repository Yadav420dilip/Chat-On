...
def delete_contact():
    res = messagebox.askyesno("Info", "Do you want to delete the contact")
    if res > 0:
...
customFont = font.Font(family="gomono", size=12)
font.families()
...
head = Label(main_frame, text='Chat_ON', font=(customFont, 24), pady=10, foreground="#ffffff", background="#3B66A5")
head.grid(row=0, column=0)

list_box = Listbox(main_frame, width=60, height=10, font=("Helvetica", 20), justify=CENTER)
list_box.bind('<Double-Button-1>', cur_select)
list_box.grid(row=1, column=0, padx=10, pady=10, sticky=E + W + N + S)
create_list_of_contact()

delete_button = Button(main_frame, text="Delete", width=10, height=1, font=("Helvetica", 10), command=lambda: delete_contact())
delete_button.grid(row=2, column=0, padx=1, pady=1)

heading = Label(main_frame, text='Create Contact', font=(customFont, 35), pady=10, foreground="#ffffff", background="#3B66A5")
...
user = StringVar()
Label(logf, text='Username: ', font=(customFont, 20), pady=5, padx=5, foreground="#ffffff", background="#3B66A5").grid(sticky=W)
Entry(logf, textvariable=user, bd=5, font=('', 15)).grid(row=0, column=1)

Id = StringVar()
Label(logf, text='User_ID: ', font=(customFont, 20), pady=5, padx=5, foreground="#ffffff", background="#3B66A5").grid(sticky=W)
...
group = LabelFrame(chat_frame, text='', font=("Helvetica", 20), padx=5, pady=5)
group.grid(row=1, column=0, columnspan=3, padx=10, pady=10, sticky=E + W + N + S)

chat_display = scrolledtext.ScrolledText(group, font=("Helvetica", 12), width=40, height=25)
chat_display.configure(state=DISABLED)
...
EntryBox = Text(EntryFrame, bd=0, bg="white", width="29", height="3", font=("Helvetica", 16))
...
SendButton = Button(EntryFrame, font=("Helvetica", 15), text="Send", width="8", height=3, bd=0,
